...
func (wp *WorkloadPolicy) ProcessPolicyStatus(
	...
	// are now in the spec.
	wp.removeStaleViolations()

	// Acknowledge any violations that have been acknowledged
	if err := wp.acknowledgeViolationsFromAnnotations(now); err != nil {
		return err
	}

	wp.Status.ObservedGeneration = wp.Generation
	wp.Status.ActiveViolationCount = int64(len(wp.Status.Violations))
	return nil
}
