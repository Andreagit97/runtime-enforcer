...
func TestPolicyNodeStatus(t *testing.T) {
	t.Run("nil policy status is treated as missing", func(t *testing.T) {
		code, msg, err := policyNodeStatus("protect", nil)
		require.NoError(t, err)
		require.Equal(t, v1alpha1.PolicyMissing, code)
		require.Equal(t, "policy not present on the node", msg)
	})

	t.Run("ready state with mode mismatch is transitioning", func(t *testing.T) {
		code, msg, err := policyNodeStatus(
			"protect",
			&pb.PolicyStatus{State: pb.PolicyState_POLICY_STATE_READY, Mode: pb.PolicyMode_POLICY_MODE_MONITOR},
		)
		require.NoError(t, err)
		require.Equal(t, v1alpha1.PolicyTransitioning, code)
		require.Empty(t, msg)
	})
}
